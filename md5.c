...
#include "md5.h"
#include "djb/byte.h" // byte_copy, byte_zero
...
void md5_update(md5Context* ctx, const unsigned char input[], unsigned int len) {
    ...
    have = (unsigned int)((ctx->count >> 3) & (MD5_BLOCK_LENGTH - 1));
    ...
    if (len >= need) {
        if (have != 0) {
            byte_copy(ctx->buffer + have, need, input);
    ...
    if (len != 0) {
        byte_copy(ctx->buffer + have, len, input);
    }
}
...
\*------------------------------------------------------------------*/
void md5_transform(unsigned int state[4], const unsigned char block[MD5_BLOCK_LENGTH]) {

    unsigned int a, b, c, d, in[MD5_BLOCK_LENGTH / 4];

#if BYTE_ORDER == LITTLE_ENDIAN
    byte_copy(in, sizeof(in), block);
#else
